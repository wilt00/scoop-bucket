...
$suppressionsPath = Join-Path $PSScriptRoot 'suppressions.json'
$suppressions = if (Test-Path $suppressionsPath) {
    Get-Content $suppressionsPath -Raw | ConvertFrom-Json
} else {
    [PSCustomObject]@{ files = @(); manifests = @() }
}
$fileExclusions = @($suppressions.files | ForEach-Object { $_.pattern })
$manifestExclusions = @($suppressions.manifests | ForEach-Object { $_.name })

. "$PSScriptRoot\Scoop-00File.ps1" -TestPath $BucketPath -ProjectFileExclusions $fileExclusions
...
Describe 'Manifest validates against the schema' {
    BeforeDiscovery {
        ...
        $manifestFiles = @(
            $manifestFiles | Where-Object { (Split-Path $_ -Leaf) -notin $manifestExclusions }
        )
    }
    BeforeAll {
